...
test.describe("Docs v0.6", () => {
  ...
  test("the header menu and the footer reach every new component", async ({ page }) => {
    ...
    for (const base of ["", "/es"]) {
      ...
      // The beta notice and the footer follow the cycle, not the previous one.
      await expect(page.locator(".docs-footer__inner")).toContainText("v0.6");
      await expect(page.locator(".docs-footer__inner")).toContainText("0.6.0");
    }
  });
