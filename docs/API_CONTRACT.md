...
| `--iv-color-hover-surface` | `rgb(8 19 16 / 0.04)` | `rgb(224 255 240 / 0.05)` | filas al hover (tabla, data table, menú), pies de tarjeta, diálogo y drawer, pestañas al hover, campo del selector al hover |
...
Además: `.iv-progress--lg` (pista de 1rem, radio completo) y local público `--iv-progress-size` (0.5rem por defecto; `--sm` 0.25rem); `--iv-card-shadow` (por defecto `var(--iv-shadow-ambient)`) y `--iv-card-shadow-hover` (por defecto `var(--iv-shadow-2)`) documentados como locales públicos de `.iv-card`. Regla de refactor: ninguna receta `color-mix()` con `--iv-color-overlay` ni literal `0 0 0 4px var(--iv-glow-primary)` queda en `src/css/components/**` tras el ciclo; `grep` lo comprueba en el gate.
