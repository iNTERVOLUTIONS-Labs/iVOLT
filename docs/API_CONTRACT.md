...
| `Lightbox` (por galería) | `open(index) close() next() prev() go(index) zoom(on?) destroy()`; `index`, `isOpen`, `items` | `loop` (true), `zoom` (true), `swipe` (true), `preload` (1), `counter` (true), `captions` (true), `closeText` («Close»), `prevText` («Previous»), `nextText` («Next»), `zoomText` («Zoom»); ver §8.20 |
