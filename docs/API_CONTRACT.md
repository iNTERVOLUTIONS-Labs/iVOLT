...
| Métodos | `open() close() select(valueOrOption) clear() destroy()`; `isOpen`, `value` (valor confirmado), `options` (lectura) |
| Teclado | ↓ abre y resalta la siguiente (con wrap), ↑ anterior, Home/End, Enter confirma la resaltada (y cierra), Esc cierra (segunda pulsación con `strict` restaura), Tab confirma la resaltada si `autoselect` y cierra; escribir filtra y abre cuando `minChars` se alcanza; clic/toque en opción confirma; clic fuera cierra |
