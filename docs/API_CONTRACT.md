...
| `iv:change` → `iv:changed` | sí / no | tabs y disclosure en modo acordeón; combobox al confirmar un valor; picker al cambiar la selección; carousel al cambiar de diapositiva; stepper al cambiar de paso | `tab`, `panel`, `previousTab` (tabs/disclosure); `value`, `option`, `previousValue` (combobox); `value`, `added`, `removed` (picker); `index`, `previousIndex`, `reason` (carousel); `index`, `previousIndex`, `reason: "trigger"\|"next"\|"prev"\|"api"\|"hash"` (stepper) |
